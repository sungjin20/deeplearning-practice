'''
몬테카를로 트리 탐색을 이용한 틱택토 인공지능을 구현한 코드임
'''
...
def is_finish(board):
	for i in range(3):
		if board[i][0] == 'O' and board[i][1] == 'O' and board[i][2] == 'O': return 'O'
		if board[i][0] == 'X' and board[i][1] == 'X' and board[i][2] == 'X': return 'X'
		if board[0][i] == 'O' and board[1][i] == 'O' and board[2][i] == 'O': return 'O'
		if board[0][i] == 'X' and board[1][i] == 'X' and board[2][i] == 'X': return 'X'
